...
void bfcMixer_Tpx(Int_t Nevents=100,
	...
	cout << "(daq=" << daqfile << ", tag=" << tagfile << ", pt_low=" << pt_low << ", pt_high=" << pt_high << ", eta_low=" << eta_low << ", eta_high=" << eta_high << ", vzlow=" << vzlow << ", vzhigh=" << vzhigh << ", vr=" << vr << ", pid=" << pid << ", mult=" << mult << ", triggers, prodName=" << prodName << ", type=" << type << ", bPythia=" << bPythia << ", fzdfile=" << fzdfile << endl;    
...
//  TString chain2Opt("NoInput,PrepEmbed,gen_T,geomT,sim_T,trs,-ittf,-tpc_daq,nodefault");
  ...
  */
  ...
  gMessMgr->QAInfo() << "Runing on  " << Nevents << " events" <<  endm;
